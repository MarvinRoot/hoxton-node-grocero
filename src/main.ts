...
function addItemToCart(itemId: number) {
  const existingItem: CartItem | undefined = state.cart.find(item => item.id == itemId)

  if (existingItem) {
    existingItem.quantity += 1
  } else {
    const itemToAdd: StoreItem | undefined = state.items.find(item => item.id == itemId)
...
function renderStoreItem(item: StoreItem) {
  ...
  const addBtn: Element | null = listItemEl.querySelector('button')
...
function renderCartItem(item: CartItem) {
  ...
  if(addBtn) addBtn.addEventListener('click', event => addItemToCart(item.id))

  const removeBtn: Element | null = listItemEl.querySelector('.remove-btn')
  if(removeBtn) removeBtn.addEventListener('click', event => removeItemFromCart(item.id))
